    ...
    <section class="content-header">
			...
			<div class="row">
				<div class="col-12">
                   ...
                   <span style='float:right'><?php if(isset($previous)){ ?> <a class='btn btn-primary btn-sm' href='<?php echo base_url()?>accounts/salesvoucher/<?php echo $previous;?>'>Prev</a> <?php } ?> <?php if(isset($next->voucherid)){ ?> <a href='<?php echo base_url()?>accounts/salesvoucher/<?php echo $next->voucherid;?>' class='btn btn-primary btn-sm'>Next</a> <?php }else { ?> <a href='<?php echo base_url()?>accounts/salesvoucher' class='btn btn-primary btn-sm'>New</a> <?php } ?></span>
					 ...
					 <div class="container">
                      <form id="voucherform" data-parsley-validate class="form-horizontal form-label-left" method='post' action="<?php echo base_url()?>sales/savecreditnote">
									<input type=hidden name='voucherid' value='<?php echo isset($voucherid)?$voucherid:""?>'>
									   <div class="container">	
										<div class="row">
										
										<div class="form-group col-md-3">
                                            <span class="control-label pull-left" for="customername">Credit Note Number<span class="required">*</span>
                                            </span>
                                            <div class="">
                                                <input type="text" style='text-align: right;' readonly value='<?php echo $credit_id;?>' id="credit_number" name="credit_number" required="required" class="form-control form-control-sm">
                                            </div>
                                        </div>
										
										<div class="form-group col-md-3 towhom">
                                            <span class="control-label" for="customername" id='typechange'>Issue To
                                            </span>
                                            <div class="">
										...
										<div class="form-group col-md-3 towhom">
                                            <span class="control-label" for="customername" id='typechange'>Type
                                            </span>
                                            <div class="">
                                                <select id="type" name="type" class="tabp required form-control form-control-sm">
                                         ...
                                         <div class="form-group col-md-3">
                                            <span for="website" class="control-label">Against Invoice No<span class="required">*</span></span>
                                            <div class="">
											<input type='hidden' id='vouchertype' name='vouchertype' value='2' >
												<select id="against_invoice_no" name="against_invoice_no" class="tabp required form-control form-control-sm">
													<option value="">Choose..</option>
														
													<?php 												
													if(isset($salesinvoiceno) && !empty($salesinvoiceno))
													{
															foreach( $salesinvoiceno as $tmp1)
															{
																...
																{
																?>
																		<option selected='selected' value='<?php echo $tmp1->invoicenumber; ?>' ><?php echo $tmp1->invoicenumber; ?></option>											
																<?php
															}
															else{ ?>
																		<option  value='<?php echo $tmp1->invoicenumber; ?>' ><?php echo $tmp1->invoicenumber; ?></option>											
																<?php 
																} 
															}
													}
										 ...
										 <div class="form-group col-md-3">
                                            <span for="website" class="control-label pull-left">Credit Date<span class="required">*</span></span>
                                            <div class="">
                                                <input type="text" name="credit_date" id="credit_date" class="form-control form-control-sm datepicker">
                                            </div>
                                        </div>
										...
										<div class="form-group col-md-3">
                                            <span class="control-label" for="customername" id='typechange'>Amount
                                            </span>
                                            <div class="">
                                                <input type="text" id="amount" name="amount" class="tabp required form-control form-control-sm" value='<?php echo isset($Amount)?$Amount:"";?>'>
                                            </div>
                                        </div>
                                        
										
										 <div class="form-group col-md-3 inamtdisplay" id='inamtdisplay' name='inamtdisplay' style='display:none;' >
										 ...
										 <div class="form-group col-md-3 inamtdisplay" id='inamtdisplay' name='inamtdisplay' style='display:none;' >
                                            <span for="website" class="control-label ">Balance Amount</span>
                                            <div class="">
										            <input type="text" id="balanceamonut" name="balanceamonut" class="tabp required form-control form-control-sm" value='0' readonly >
                                    
                                            </div>
                                        </div>
										
										<div class="form-group col-md-3 inamtdisplay" id='inamtdisplay' name='inamtdisplay' style='display:none;' >
                                            <span for="website" class="control-label ">Invoice Date</span>
                                            <div class="">
										        <input type="text" id="invoicedate" name="invoicedate" class="tabp required form-control form-control-sm datepicker" readonly >
		...
		<div class="row">
			...
			<div class="col-md-1" style='padding-left:10px;'>
				<a style='cursor:pointer;padding:0px 0px' id='addNew' class="btn btn-success">Add row</a>
			</div>
            </div>			
		</div>
		...
		?>
			<div class="form-group">
               <div class="row">			
				...
				<div class="col-md-1"  <?php if($i == 1){ ?>style='padding-left:30px;margin-top:7px' <?php } ?> >
					
					
					<?php
						if($i == 1){
					?>
					<a style='cursor:pointer;padding:0px 0px' id='addNew' class="btn btn-success">Add(F2)</a>
					<?php
						} else {
					?>
		...
		<div class="form-group">	
			...
			<div class="col-md-12" align='center'>
				<textarea placeholder='Narration' class='tabp form-control form-control-sm' name='narration' id='narration'><?php echo isset($narration)?$narration:""?></textarea>
			</div>				
		</div>	
										<Br>
...
<script type='text/javascript'>
		  $(document).ready(function () {
			   ...
			   $("#subcategory").change(function(){				  
				   ...
				   $.ajax({
					  ...
					  success: function( data ) {
						  if(data.success == true){
							new PNotify({
								title: 'Voucher',
								text: 'Account Name Created Successfully',
								type: 'success'
							});
							$("#closeclick").click();
						  }
				...
				$("#against_invoice_no").val(<?php echo $invoicenum;?>);
			     $("#against_invoice_no").change(function(){
				   $.ajax({
					  ...
					  success: function( data ) {
						...
						{
							$('.inamtdisplay').show();
							$('#invamt').val(data.invoiceamount);
							$('#balanceamonut').val(data.balanceamount);
